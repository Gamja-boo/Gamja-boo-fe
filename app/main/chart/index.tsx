...
import { categorizeMonthlyData } from '@/app_utils/finance/categorizeMonthlyData';
...
const TEXT_COLORS = ['#329257', '#329257', '#329257', '#FCFFF6', '#FCFFF6', '#FCFFF6'];
...
export default function ChartScreen() {
  ...
  const monthlyData = categorizeMonthlyData(
    yearlyData[parseInt(monthOfCurrentInfo, 10) - 1],
    isExpenditure ? 'E' : 'I',
  );
  const totalAmount =
    (isExpenditure
      ? yearlyData[parseInt(monthOfCurrentInfo, 10) - 1]?.totalSpent
      : yearlyData[parseInt(monthOfCurrentInfo, 10) - 1]?.totalIncome) ?? 100;
  const coloredData =
    monthlyData?.map((item, index) => {
      return {
        ...
        percentage: Math.round((item.amountSum / totalAmount) * 100),
      };
    }) ?? [];
  ...
  }, [hideDoubleArrowDown, hideAnim]);

  const chartAnimation = useCallback(() => {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        >
          {showDetail ? (
            <View style={[styles.viewDetailButton, { backgroundColor: '#75E88C' }]}>
              <Text style={[styles.text3, { color: '#FFFFFF' }]}>이런 내역이 있었어요!</Text>
              <ArrowUp fill="#FFFFFF" height={screenWidth * 0.06} width={screenWidth * 0.06} />
            </View>
          ) : (
            <View style={styles.viewDetailButton}>
              <Text style={styles.text3}>내역을 자세히 볼까요?</Text>
              <ArrowDown fill="#329257" height={screenWidth * 0.06} width={screenWidth * 0.06} />
            </View>
          )}
        </TouchableOpacity>
        {showDetail ? (
            ...
            >
              <View style={styles.modalBody}>
                <View style={styles.detailModal}>
                  <Text style={styles.title}>조금 더 들여다보면 이런 배경이 있었어요</Text>
                  <View style={styles.attributeContainer}>
                    <View style={[styles.attribute, { marginRight: screenWidth * 0.05 }]}>
                      <Text style={styles.attributeText}>날짜</Text>
                    </View>
                    <View style={styles.attribute}>
                      <Text style={styles.attributeText}>지출배경</Text>
                    </View>
                    <View style={[styles.attribute, { marginLeft: screenWidth * 0.05 }]}>
                  ...
                  >
                    {coloredData[selectedCategryIndex]?.transactions.map((item) => {
                      return (
                        <View
                        ...
                        >
                          ...
                          <Text style={styles.descriptionText}>{item.background}</Text>
